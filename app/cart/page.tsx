...
const Cart = () => {
	...
	return (
		<>
			<head>
				<title>Winkelwagen - Windowslicenties.nl</title>
				<meta
					name='description'
					content='Bekijk producten in uw Winkelwagen. Heeft u nog geen producten in uw winkelwagen? Zoek dan Windows 10 licenties en Office 2019 licenties om aan te schaffen!'
				/>
			</head>
						...
						{cartItems.map((item) => (
							<div key={item.product.id} className='my-5'>
								<div className='flex flex-col lg:flex-row gap-4 items-center justify-between mb-6 px-4'>
									...
									/>
									<p className='text-lg'>
										&#8364;{(parseFloat(item.product.discount_price) * item.quantity).toFixed(2)}
									</p>
								</div>
								<Separator />
							</div>
							...
							<div>
								...
								<p className='flex justify-between w-3/4 items-center mt-2'>
									...
									<span className='text-xs text-gray-400'>
										...
										{(
											cartItems.reduce(
												(acc, item) => acc + parseFloat(item.product.discount_price) * item.quantity,
												0
											) * 0.21
										).toFixed(2)}{" "}
										BTW)
